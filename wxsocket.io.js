...
class WxSocketIO {
  connect(url, opts) {
    ...
    // opts.retryTimes = opts.retryTimes || 3
    // opts.retryInterval = opts.retryInterval || 3
    let qs = {}
    Object.assign(qs, opts)
    delete qs.path
    delete qs.retryTimes
    delete qs.retryInterval
    let qsStr = serialize(qs)
    return new Promise((resolve, reject) => {
      wx.onSocketOpen(_ => {
        console.log('WX:SocketOpen', _)
        this.isConnected = true
        // this.ping()
        resolve(_)
      })
      wx.onSocketError(error => {
        console.log('WX:SocketError', error)
        if (this.isConnected) {
      ...
      wx.onSocketMessage(
        message => {
          console.log('WX:OnSocketMessage', message)
          this._handleMessage(message)
        }
      )
      wx.onSocketClose(result => {
        console.log('WX:SocketClose', result)
        if (this.isConnected) {
      ...
      let _url = `${url}/${opts.path}/?EIO=3&transport=websocket&${qsStr}`
  ...
  _handleMessage({ data }) {
    // console.info('_handleMessage:', data)
